import { Product } from "../models/product";

export const PRODUCTS_DATA: Product[] = [
    {
      id: 1,
      name: 'Beer - Moosehead',
      description:
        'nisi nam ultrices libero non mattis pulvinar nulla pede ullamcorper augue a suscipit nulla elit ac',
      imageUrl: 'http://dummyimage.com/300x200.png/5fa2dd/ffffff',
      price: 46.25,
    },
    {
      id: 2,
      name: 'Bread - Crusty Italian Poly',
      description:
        'felis fusce posuere felis sed lacus morbi sem mauris laoreet ut rhoncus aliquet pulvinar sed nisl nunc rhoncus dui vel',
      imageUrl: 'http://dummyimage.com/300x200.png/cc0000/ffffff',
      price: 29.2,
    },
    {
      id: 3,
      name: 'Food Colouring - Green',
      description:
        'mi pede malesuada in imperdiet et commodo vulputate justo in blandit ultrices enim lorem ipsum dolor',
      imageUrl: 'http://dummyimage.com/300x200.png/5fa2dd/ffffff',
      price: 89.18,
    },
    {
      id: 4,
      name: 'Veal - Nuckle',
      description:
        'et ultrices posuere cubilia curae nulla dapibus dolor vel est donec odio justo sollicitudin ut suscipit a',
      imageUrl: 'http://dummyimage.com/300x200.png/5fa2dd/ffffff',
      price: 66.84,
    },
    {
      id: 5,
      name: 'Soup - Campbells Asian Noodle',
      description:
        'pede justo eu massa donec dapibus duis at velit eu est congue',
      imageUrl: 'http://dummyimage.com/300x200.png/ff4444/ffffff',
      price: 67.88,
    },
    {
      id: 6,
      name: 'Seedlings - Mix, Organic',
      description:
        'dictumst maecenas ut massa quis augue luctus tincidunt nulla mollis molestie lorem quisque ut erat curabitur gravida nisi',
      imageUrl: 'http://dummyimage.com/300x200.png/dddddd/000000',
      price: 24.87,
    },
    {
      id: 7,
      name: 'Pork - Tenderloin, Fresh',
      description:
        'fusce lacus purus aliquet at feugiat non pretium quis lectus suspendisse potenti in eleifend quam a odio',
      imageUrl: 'http://dummyimage.com/300x200.png/5fa2dd/ffffff',
      price: 98.69,
    },
    {
      id: 8,
      name: 'Wine - Tribal Sauvignon',
      description:
        'amet erat nulla tempus vivamus in felis eu sapien cursus vestibulum proin eu',
      imageUrl: 'http://dummyimage.com/300x200.png/cc0000/ffffff',
      price: 86.74,
    },
    {
      id: 9,
      name: 'Juice - V8, Tomato',
      description:
        'turpis enim blandit mi in porttitor pede justo eu massa donec dapibus duis at velit eu est congue elementum',
      imageUrl: 'http://dummyimage.com/300x200.png/5fa2dd/ffffff',
      price: 97.84,
    },
    {
      id: 10,
      name: 'Pork - Side Ribs',
      description:
        'in tempor turpis nec euismod scelerisque quam turpis adipiscing lorem vitae mattis nibh ligula nec sem duis',
      imageUrl: 'http://dummyimage.com/300x200.png/cc0000/ffffff',
      price: 62.0,
    },
  ];
  